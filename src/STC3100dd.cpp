...
STC3100::STC3100(uint8_t sample_rate, uint16_t resistor_value){
...
bool STC3100::start(){
    ...
    Serial.print("STC3100 STC3100_REG_CTRL 0x");
    ...
    Serial.print("STC3100 After initCTL 0x");
    Serial.println(getReadingWire(STC3100_REG_MODE), HEX);

    return true;
}

STC3100::fgValues_t STC3100::readValues(){
...
float STC3100::rawToCharge_mAhr(uint16_t rawReg) {
    return ((float) rawReg) *STC3100_mAHr_FACTOR;
} 

float STC3100::rawToCurrent_mA(uint16_t rawReg) {
...
float STC3100::readTemperature_C(){
    float t = ((uint16_t)getReadingWire(STC3100_REG_TEMPERATURE_LOW));
    return t;
}

bool STC3100::readSerialNumber(uint8_t *serialNum){
    Wire.beginTransmission(BUS_ADDRESS);
...
void STC3100::getAllReg(uint8_t *dataReg){
    Wire.beginTransmission(BUS_ADDRESS);
    Wire.write(STC3100_REG_CHARGE_LOW);
    Wire.endTransmission();
    Wire.requestFrom(BUS_ADDRESS, STC3100_REG_LEN);
...
uint16_t STC3100::getReadingWire(uint8_t reg){
    Wire.beginTransmission(BUS_ADDRESS);
    Wire.write(reg);
    Wire.endTransmission();
...
/* FUT not tested */
void STC3100::writeByteWire(uint8_t reg, uint8_t value){
    Wire.beginTransmission(BUS_ADDRESS);
    Wire.write(reg);
    Wire.write(value);
    Wire.endTransmission();
...
uint8_t STC3100::crc8calc(const void * data, size_t size){
    uint8_t val = 0;
